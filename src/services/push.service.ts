...
export class PushService {
  ...
  private static handleTickets = async (messages: ExpoPushMessage[], tickets: ExpoPushTicket[]) => {
    for (let i = 0; i < tickets.length; i++) {
      const ticket = tickets[i];
      const token = messages[i]?.to;

      if (ticket.status === "error" && token && typeof token === "string") {
        console.error("[PushService] Push ticket error:", ticket.details || ticket.message);
